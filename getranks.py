...
def lambda_handler(event, context):
    ...
    try:
        ...
        for each_user_index in range(0, len(scan_result['Items'])):
            ...
            xp_userindex_dict[skill_xp] = each_user_index
            
            print(str(xp_userindex_dict))
        ...
        sorted_xp_keys = sorted(xp_userindex_dict.keys(), reverse=True)
        
        print(str(sorted_xp_keys))
        for i, each_user_xp in enumerate(sorted_xp_keys):
            each_user_index = xp_userindex_dict[each_user_xp]
